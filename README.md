...
8. MyPage에서 등록된 물건과 거래상태를 조회할 수 있다.
.
비기능
.
트랜젝션
1. 거래완료된 물건은 채팅 요청이 불가능해야 한다. 
2. 예약 컨펌상태인 item은 삭제할 수 없다.

장애격리
1. 결제시스템이 수행되지 않더라도 reservation시스템은 365일 24시간 운행되어야 한다. 
2. payment시스템이 과중되면 사용자에게 결재를 바로 받지않고, 잠시 후에 하도록 유도한다.

성능
1. 모든 제품에 대한 정보 및 예약 상태 등을 한번에 확인할 수 있어야 한다 (CQRS)
...
![image](https://user-images.githubusercontent.com/15603058/119300858-6c21b300-bc9c-11eb-9b3f-c85aff51658f.png)
    ...
    - 도메인 서열 분리 
        - Core Domain:  revervation, item : 없어서는 안될 핵심 서비스이며, 연간 Up-time SLA 수준을 99.999% 목표, 배포주기는 reservation 의 경우 1주일 1회 미만, room 의 경우 1개월 1회 미만
        - Supporting Domain:   review, viewpage : 경쟁력을 내기위한 서비스이며, SLA 수준은 연간 60% 이상 uptime 목표, 배포주기는 각 팀의 자율이나 표준 스프린트 주기가 1주일 이므로 1주일 1회 이상을 기준으로 함.
